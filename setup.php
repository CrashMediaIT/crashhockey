...
// Retrieve admin creation status from session
$admin_created_session = isset($_SESSION['admin_created']) ? $_SESSION['admin_created'] : false;

// Handle step progression
// Check if we're just moving to step 4 (no form fields submitted yet)
if (isset($_POST['step']) && $_POST['step'] == 4 && !isset($_POST['smtp_host'])) {
    // Just displaying step 4 form (SMTP), not processing it
    $step = 4;
} else {
            ...
            try {
                ...
                if (!file_exists($schema_file)) {
                    ...
                    if (empty($schema)) {
                        ...
                        // MariaDB/MySQL compatibility settings
                        $pdo->exec("SET FOREIGN_KEY_CHECKS = 0");
                        $pdo->exec("SET SQL_MODE = 'NO_AUTO_VALUE_ON_ZERO'");
                        $pdo->exec("SET NAMES utf8mb4");
                        $pdo->exec("SET CHARACTER SET utf8mb4");
                        
                        // Remove comments first (more aggressive cleaning)
                        $schema = preg_replace('/^--.*$/m', '', $schema); // Remove single-line comments
                        $schema = preg_replace('/\/\*.*?\*\//s', '', $schema); // Remove multi-line comments
                        $schema = preg_replace('/^\s*\n/m', '', $schema); // Remove empty lines
                        
                        // Split statements more reliably - split on semicolon followed by newline or end of string
                        ...
                        $successful_count = 0;
                        $total_statements = 0;
                        $debug_log = []; // Debug what's being executed
                        ...
                        foreach ($statements as $statement) {
                            ...
                            $statement_upper = strtoupper(substr($statement, 0, 50));
                            if (strpos($statement_upper, 'CREATE TABLE') !== false || 
                                strpos($statement_upper, 'CREATE INDEX') !== false ||
                                strpos($statement_upper, 'CREATE UNIQUE INDEX') !== false) {
                        ...
                        // Execute CREATE TABLE statements first with detailed logging
                        foreach ($create_statements as $idx => $statement) {
                            try {
                                ...
                                // Extract table name for debug
                                if (preg_match('/CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?`?(\w+)`?/i', $statement, $matches)) {
                                    $debug_log[] = "✓ Created table: {$matches[1]}";
                                }
                            } catch (PDOException $e) {
                                ...
                                $debug_log[] = "✗ FAILED to create table: {$table_name} - " . $e->getMessage();
                                
                                $failed_statements[] = [
                                    'statement' => substr($statement, 0, 300) . (strlen($statement) > 300 ? '...' : ''),
                                    'error' => $e->getMessage(),
                                    'type' => 'CREATE',
                                    'table' => $table_name
                                ];
                        ...
                        $_SESSION['schema_stats'] = [
                            ...
                            'debug_log' => $debug_log,
                            'create_count' => count($create_statements),
                            'insert_count' => count($other_statements)
                        ];
...
// Step 3: Create first admin account (moved from step 4)
            ...
            if ($host_match && $name_match && $user_match) {
                ...
                try {
                    ...
                    if ($result->rowCount() === 0) {
                        ...
                        // Create admin account with is_verified = 1 (no email verification required)
                        ...
                        $admin_created = true;
                        $_SESSION['admin_created'] = true; // Persist in session
                        // Move to step 4 (SMTP configuration)
                        $step = 4;
                    }
...
// Step 4: Configure and test SMTP (moved from step 3)
            ...
            if ($host_match && $name_match && $user_match) {
                ...
                try {
                    ...
                    if (!$error) {
                        ...
                        if ($test_result) {
                            // Create lock file to complete setup
    ...
    <div class="setup-container">
        ...
        <?php if (!$success): ?>
            ...
            <?php elseif ($step == 2.5): ?>
                ...
                <?php if (!empty($stats['debug_log'])): ?>
                <div style="background: #0f1419; border: 1px solid #2a3f5f; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
                    <h3 style="font-size: 14px; color: #6ba0ff; text-transform: uppercase; margin-bottom: 15px; letter-spacing: 0.5px;">
                        <i class="fas fa-bug"></i> Debug Log
                    </h3>
                    <div style="font-size: 11px; color: #94a3b8; margin-bottom: 10px;">
                        CREATE statements: <?= $stats['create_count'] ?? 0 ?> | INSERT/ALTER statements: <?= $stats['insert_count'] ?? 0 ?>
                    </div>
                    <div style="max-height: 250px; overflow-y: auto; background: #06080b; padding: 15px; border-radius: 4px; font-family: monospace; font-size: 11px;">
                        <?php foreach ($stats['debug_log'] as $log_entry): ?>
                            <?php
                            $is_success = strpos($log_entry, '✓') !== false;
                            $is_failure = strpos($log_entry, '✗') !== false;
                            $color = $is_success ? '#00ff88' : ($is_failure ? '#ff4444' : '#64748b');
                            ?>
                            <div style="color: <?= $color ?>; margin-bottom: 3px;"><?= htmlspecialchars($log_entry) ?></div>
                        <?php endforeach; ?>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <?php else: ?>
                    <div class="alert alert-success" style="margin-bottom: 20px;">
                        ...
                        <strong>SUCCESS:</strong> All required database tables have been created successfully! You can now proceed to create your admin account.
                    </div>
            ...
            <?php elseif ($step == 3): ?>
                ...
                <p style="color: #94a3b8; margin-bottom: 20px; line-height: 1.6;">
                    Create your first administrator account to manage the system. <strong>Note:</strong> This account will be immediately active (no email verification required).
                </p>
            ...
            <?php elseif ($step == 4): ?>
                ...
                <p style="color: #94a3b8; margin-bottom: 20px; line-height: 1.6;">
                    Configure email settings to enable verification emails and notifications. This is the final step!
                </p>
                ...
                <form method="POST">
                    ...
                    <button type="submit" class="btn">
                        <i class="fas fa-envelope"></i> Complete Setup & Test SMTP
                    </button>
                </form>
            <?php endif; ?>
        <?php else: ?>
            <div class="success-card">
                ...
                <p>
                    Crash Hockey has been successfully configured! Your admin account has been created and SMTP settings have been saved.
                </p>
                <p style="font-size: 14px; color: #94a3b8; margin-top: 15px;">
                    <strong>What's next?</strong><br>
                    • Log in with your admin credentials<br>
                    • Configure additional system settings<br>
                    • Start managing your hockey training platform
                </p>
                <a href="login.php">
